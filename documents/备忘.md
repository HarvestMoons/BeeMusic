...
select vdisk file="E:\Docker\wsl-data\DockerDesktopWSL\data\ext4.vhdx"
attach vdisk readonly
compact vdisk
detach vdisk
exit

一个可用的 Docker 镜像加速器列表：
"registry-mirrors": [
"https://docker.1ms.run",
"https://docker.1panel.live/"
]

github actions的秘钥格式要求（用cat ~/.ssh/id_rsa在本地取，确保对应公钥已上传到服务器，用ssh -i ~/.ssh/id_rsa
root@8.155.47.138 -p 22验证，不用输入密码就对了）：
顶部一行必须是 -----BEGIN OPENSSH PRIVATE KEY-----
底部一行必须是 -----END OPENSSH PRIVATE KEY-----

已经安装CLI代码行数统计工具cloc，用法：去根目录下运行`cloc .`

更新https证书后，把证书下载后放到Ice\certs\beemusic.fun.key（pem），服务器也要放一份