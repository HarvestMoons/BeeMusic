...
在音乐播放器中加入的彩蛋功能，用户可以随机打捞（获取）一张梗图，支持下载、管理（同步/删除）。底层基于 **阿里云 OSS** 存储海量图片，
**Spring Boot + Vue 3** 实现业务逻辑。
...
* **问题**：OSS 上有数万张图片，同步元数据到数据库时，若每张图片都查询一次 "是否存在" (`SELECT count` 或 `exists`)，将产生 *
  *N+1 次数据库交互**。
...
* **问题**：
    ...
    * 硬编码 URL 过期时间导致维护困难。
* **优化**：
    * **动态签名**：数据库只存对象的 `Key` (文件名)，不存完整 URL。每次请求时，后端使用 `OSSClient` 动态生成 **预签名 URL (
      Presigned URL)**。
